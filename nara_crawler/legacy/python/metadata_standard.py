...
class StandardMetadataScanner:
            ...
            try:
                ...
                if isinstance(data, dict):
                    if data.get('description') == '해당 데이터는 존재하지 않습니다.':
                        return False
                    if data.get('title') or data.get('organization') or data.get('standardType'):
                        return False
            ...
            if response.status_code == 200:
                ...
                if (
                    ...
                    return {
                        ...
                        'has_data': False,
                        'status': 'not_found',
                        'error': 'Standard 메타데이터 없음',
                        'retry_count': retry_count
                    }
                
                # Standard 데이터 존재 여부 확인
                has_data = bool(data)
                ...
                # 표준 타입 통계 업데이트
                if standard_info['standard_type']:
                    standard_type = standard_info['standard_type'].upper()
                    self.results['standard_types'][standard_type] = self.results['standard_types'].get(standard_type, 0) + 1
                
                if has_data and (standard_info['standard_url'] or standard_info['title']):
                    self.results['standard_numbers'].append(num)
    ...
    def scan_range(self):
            ...
            with tqdm(total=total_numbers, desc="스캔 진행") as pbar:
                for future in concurrent.futures.as_completed(future_to_num):
                    ...
                    if pbar.n % 100 == 0:
                        ...
                        pbar.set_postfix({
                            '표준있음': self.results['with_data'],
                            '표준없음': self.results['without_data'],
        ...
        # 표준 번호 정렬
        self.results['standard_numbers'].sort()
    ...
    def save_results(self, output_dir="/data/metadata_results"):
        """스캔 결과 저장"""
        # /data/metadata_results/standard 폴더 생성
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            json.dump({
                ...
                'standard_data_found': self.results['with_data'],
                'standard_data_not_found': self.results['without_data'],
                ...
                'standard_types': self.results['standard_types'],
                'scan_time': self.results.get('scan_time', {}),
                'standard_count': len(self.results['standard_numbers'])
            }, f, ensure_ascii=False, indent=2)
        
        # 2. 표준 데이터가 있는 번호만 별도 저장
        standard_numbers_file = os.path.join(type_dir, "standard_numbers.json")
        with open(standard_numbers_file, 'w', encoding='utf-8') as f:
            json.dump({
                'standard_numbers': self.results['standard_numbers'],
                'count': len(self.results['standard_numbers']),
                'scan_info': {
                    'range': f"{self.start_num}-{self.end_num}"
                }
            }, f, ensure_ascii=False, indent=2)
        
        # 3. 표준 번호 목록을 텍스트 파일로도 저장
        standard_list_file = os.path.join(type_dir, "standard_numbers.txt")
        with open(standard_list_file, 'w', encoding='utf-8') as f:
            for num in self.results['standard_numbers']:
                f.write(f"{num}\n")
        
        # 4. 상세 표준 메타데이터 저장 (표준이 있는 것만)
        standard_metadata_file = os.path.join(type_dir, "standard_metadata.json")
        standard_metadata = {
            num: details for num, details in self.results['details'].items()
            if details.get('has_data', False)
        }
        with open(standard_metadata_file, 'w', encoding='utf-8') as f:
            json.dump(standard_metadata, f, ensure_ascii=False, indent=2)
        
        # 5. 표준 타입별 번호 목록 저장
        for standard_type, count in self.results['standard_types'].items():
            if count > 0:
                type_numbers = []
                for num, details in self.results['details'].items():
                    if details.get('standard_type', '').upper() == standard_type:
                        type_numbers.append(num)
                
                if type_numbers:
                    type_file = os.path.join(type_dir, f"standard_type_{standard_type}.json")
        ...
        return {
            ...
            'standard_numbers_file': standard_numbers_file,
            'standard_list_file': standard_list_file,
            'standard_metadata_file': standard_metadata_file,
    ...
    def print_summary(self):
        """스캔 결과 요약 출력"""
        print("\n" + "=" * 60)
        print("📊 Standard 메타데이터 스캔 완료!")
        print("=" * 60)
        print(f"🔍 스캔 범위: {self.start_num:,} ~ {self.end_num:,}")
        print(f"📋 총 스캔: {self.results['total']:,}개")
        print(f"✅ 표준 있음: {self.results['with_data']:,}개 ({self.results['with_data'] / self.results['total'] * 100:.1f}%)")
        print(f"❌ 표준 없음: {self.results['without_data']:,}개")
        ...
        # 표준 타입별 통계
        if self.results['standard_types']:
            print(f"\n📋 표준 타입별 분포:")
            sorted_types = sorted(self.results['standard_types'].items(), key=lambda x: x[1], reverse=True)
            for standard_type, count in sorted_types[:10]:
                percentage = count / self.results['with_data'] * 100 if self.results['with_data'] > 0 else 0
                print(f"   - {standard_type}: {count}개 ({percentage:.1f}%)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='공공데이터포털 Standard 메타데이터 스캐너',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
예제:
  python metadata_standard.py -s 1 -e 1000
  python metadata_standard.py -s 1 -e 10000 -w 100
  python metadata_standard.py -s 1 -e 100000 -o standard_scan_results
  python metadata_standard.py -s 1 -e 1000 -r 5 -d 2.0 --timeout 10
        """
    )
    ...
    # 스캐너 생성 및 실행
    scanner = StandardMetadataScanner(
    ...
    try:
        ...
        print(f"\n📁 결과 위치: {args.output}/standard/")
        

                
    except KeyboardInterrupt:
        print(f"\n\n⚠️  Standard 스캔이 사용자에 의해 중단되었습니다.")
        sys.exit(1)
    except Exception as e:
        print(f"\n❌ Standard 스캔 중 오류 발생: {str(e)}")
        sys.exit(1)
